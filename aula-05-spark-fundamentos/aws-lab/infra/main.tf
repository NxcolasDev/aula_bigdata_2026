...
# -----------------------------------------------------------------------------
resource "terraform_data" "bucket_lab" {
  input = var.bucket_nome

  provisioner "local-exec" {
    command = "aws s3api head-bucket --bucket ${self.input} 2>/dev/null || aws s3api create-bucket --bucket ${self.input} --region ${var.regiao}"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "aws s3 rm s3://${self.input} --recursive || true\naws s3api delete-bucket --bucket ${self.input} --region us-east-1 || true"
  }
}
...
resource "aws_s3_bucket_public_access_block" "lab" {
  ...
  depends_on = [terraform_data.bucket_lab]
}
